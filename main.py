...
def count_computer(list_class_computer):
    '''统计class_buy_computer中yes、no的次数，返回k1、k2'''
...
def self_count(age, class_computer, keyword):
    '''如果age等于keyword则判断class_computer是yes或no，并返回值'''
    if age == keyword:
...
def count_age_computer(list_age, list_class_computer):
    '''统计age分别为youth、middle_age、senior时，class_computer是yes和no的次数，返回a1,a2,b1,b2,c1,c2'''
    a1 = 0
    a2 = 0
    b1 = 0
    b2 = 0
...
if __name__ == '__main__':
    D = read_excel()#读取excel内容到列表D
    a, b, c = count_age(D[1])#统计age中youth、middle_age、senior次数，返回a、b、c
    k1, k2 = count_computer(D[5])#统计class_buy_computer中yes、no的次数，返回k1、k2
    a1, a2, b1, b2, c1, c2 = count_age_computer(D[1], D[5])#统计age分别为youth、middle_age、senior时，class_computer是yes和no的次数，返回a1,a2,b1,b2,c1,c2
    info=cal_2(k1,k2)#根据公式计算info
    infoage = cal_1(a, b, c, 'a') * cal_2(a1, a2) + \
              cal_1(a, b, c, 'b') * cal_2(b1, b2) + \
              cal_1(a, b, c, 'c') * cal_2(c1, c2)
    #根据公式计算infoage
    gain=info-infoage
    print('info={}\ninfoage={}\nGain(age)={}'.format(info,infoage,gain))
